...
class Medium(Model):
    ...
    short_name = Column(String(4),
                 info={'description': '\"mx\" , where x is the medium id',
                       'label': 'Medium ID'})
    full_name = Column(String(100),
                 info={'description': 'Full name of the medium',
                       'label': 'Medium name'})
...
class Exposure(Model):
    ...
    chemical_id = Column(Integer, ForeignKey('chemical.id'),
                         info= {'label':'Chemical'})
    nanomaterial_id = Column(Integer, ForeignKey('nanomaterial.id'),
    ...
    medium_id = Column(Integer, ForeignKey('medium.id'),
                         info= {'label':'Medium'})
    fbs = Column(Float,
                 info={'description': 'Percent of fetal bovine serum used in medium',
                       'label': 'FBS'})
    timepoint = Column(Integer,
                 info={'description': 'Time in h between exposure start and endpoint measurement',
                       'label': 'Timepoint [h]'})
    replicates = Column(Integer,
                  info={'description': 'Number of biological replicates',
                        'label': 'No. of replicates'})
    plate_size = Column(Integer,
                  info={'description': 'Number of wells in plate',
                        'label': 'Plate Size'})
    dosing = Column(String(10),
                         info= {'label':'Dosing',
                                'description': 'direct or indirect'})
    insert = Column(Boolean,
                  info={'description': 'Was an insert used?',
                        'label' : 'Insert'})
    passive_dosing = Column(Boolean,
                         info= {'label':'Passive Dosing'})
    solvent_id = Column(Integer,ForeignKey('solvent.id'),
                        info={'label': 'Solvent'})
    id_string = Column(String(200))
    cells_seeded = Column(Integer,
                         info= {'label':'Cells seeded'})
    volume = Column(Float,
                         info= {'label':'Volume'})
    conc_determination = Column(String(20),
                  info={'label':'Conc. determination',
                        'description': 'Are the given concentration values nominal or measured?'})
    experimenter_id = Column(Integer, ForeignKey('person.id'),
                         info= {'label':'Experimenter'})
    corresponding_author_id = Column(Integer, ForeignKey('person.id'),
    ...
    institution_id = Column(Integer,ForeignKey('institution.id'),
                         info= {'description':'Institution where the experiment was performed',
                                'label':'Institution'})
    ...
    institution = relationship(Institution)
    experimenter = relationship("Person", foreign_keys="Exposure.experimenter_id",
                                primaryjoin = Person.id == experimenter_id)
    corresponding_author = relationship("Person",
                                        foreign_keys="Exposure.corresponding_author_id",
                                        primaryjoin = Person.id == corresponding_author_id
                                        )
    
    solvent = relationship(Solvent)
    estimated = relationship("Estimated", cascade="all, delete-orphan",uselist=False,
                             back_populates="exposure")
    dose_response = relationship("Dose_response", cascade = "all, delete", backref = "exposure",single_parent=True)
        ...
        if not self.estimated.ec50:
            if self.estimated.exceeds_direction:
                ...
                if self.estimated.exceeds_direction == "greater":
                    return "> " + format_helper(self.estimated.exceeds_value,precision)
                else:
                    return "< " + format_helper(self.estimated.exceeds_value,precision)
            else:
                return ""
...
class Dose_response(Model):
    ...
    #exposure = relationship(Exposure, back_populates="dose_response", cascade="all, delete-orphan")
...
class Estimated(Model):
    ...
    exposure = relationship(Exposure, back_populates="estimated", cascade="all, delete")
