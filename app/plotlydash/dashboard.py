...
def range_slider(ename,df,colname,step):
    return html.Div(dcc.RangeSlider(
            ...
            marks={"{:.{}f}".format(x, 0 ): "{:.{}f}".format(x, 0 ) for x in np.arange(-10,100)},
            step=step
        ), style={'width': '100%', 'padding': '5px 0px 20px 0px'})
...
def create_dashboard(server):
    ...
    chemical_property_indicators = ['logKow','solubility','henry\'s law constant']
...
def get_chemprop(df,name):
    ...
    return dff.drop("priority",axis=1).rename(columns={"variable": name + "_determination", "value": name})
...
def init_callbacks(dash_app,df):
    ...
    def update_graph(xaxis_column_name, yaxis_column_name,
                     xaxis_type, yaxis_type,log_kow,endpoint,chemical,timepoint,
                     xcome,ycome,uml):
        ...
        xfil = xdat[['chemical_name',"cas_number","endpoint","experimental_log_kow","conc_determination","ec50"]]
        yfil = ydat[['chemical_name',"cas_number","endpoint","conc_determination","ec50"]]
    ...
    @dash_app.callback(
        dash.dependencies.Output('crossfilter-indicator-scatter-chemical', 'figure'),
        [dash.dependencies.Input('crossfilter-xaxis-column-chemical', 'value'),
         dash.dependencies.Input('crossfilter-yaxis-column-chemical', 'value'),
         dash.dependencies.Input('crossfilter-xaxis-type-chemical', 'value'),
         dash.dependencies.Input('crossfilter-yaxis-type-chemical', 'value'),
         dash.dependencies.Input('crossfilter-kow--rangeslider', 'value'),
         dash.dependencies.Input('crossfilter-chemical', 'value'),
    ...
    def update_graph_chemical(xaxis_column_name, yaxis_column_name,
                     xaxis_type, yaxis_type,log_kow,chemical,cell_line,timepoint,
                     xcome,ycome,uml):
        #dff = df[df['experimental_log_kow'] == experimental_log_kow]
        lox = (df['user_corrected_experimental_log_kow'] >= log_kow[0])  \
            | (df['experimental_log_kow'] >= log_kow[0])  \
    ...
    @dash_app.callback(
        dash.dependencies.Output('ec50-logkow', 'figure'),
    ...
    def update_graph_kow(xaxis_type,yaxis_type,log_kow,chemical,chemical_property,
                         cell_line,timepoint,uml):
        lox = (df['user_corrected_experimental_log_kow'] >= log_kow[0])  \
            | (df['experimental_log_kow'] >= log_kow[0])  \
            | (df['estimated_log_kow'] >= log_kow[0])
        hix = (df['user_corrected_experimental_log_kow'] <= log_kow[1])  \
        ...
        return {
            'data': [dict(
                x=dff[chemical_property_map[chemical_property]],
                y=dff['ec50'],
