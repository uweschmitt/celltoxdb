...
from app.models import Exposure, Chemical, Estimated, Cell_line, Endpoint, Solvent, Medium
...
filter_mapping = {
    ...
    "endpoint" : lambda x: Endpoint.short_name.in_(tuple(x)),
    ...
    "medium" : lambda x: Medium.full_name == x if x =="L15/ex" else Medium.full_name!= "L15/ex" ,
    ...
    "fbs": lambda x: Exposure.fbs > 0 if x == '1' else Exposure.fbs == 0,
...
sort_combinations = list(itertools.product(order_map.keys(),order_dir_map.keys()))
