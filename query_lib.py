...
def get_database_readable(db):
    ...
    querydb = db.session.query(Exposure) \
      .outerjoin(Estimated) \
      .outerjoin(Chemical,Exposure.chemical_id == Chemical.id) \
      .outerjoin(Nanomaterial,Exposure.nanomaterial_id == Nanomaterial.id) \
      .outerjoin(Cell_line) \
      .join(Medium) \
      .join(Solvent) \
      .join(Endpoint) \
      ...
      .outerjoin(ca_person,Exposure.corresponding_author_id == ca_person.id).with_entities(
    ...
    Nanomaterial.size.label("nanomaterial_size"),
    Nanomaterial.treatment.label("nanomaterial_treatment"),
    Nanomaterial.core.label("nanomaterial_core"),
    Nanomaterial.coating.label("nanomterial_coating"),
